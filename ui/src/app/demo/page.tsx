...
import { demoObjectType } from "src/config";
...
export default function Home() {
  ...
  const getObjectfromAddress = async (address: string) => {
    if (address) {
      const coco_id = await getOwnedCocoObjectId(address, demoObjectType);
      if (coco_id !== "") {
        setObjectid(coco_id);
      }
    }
  ...
  const beginZkLogin = async (provider: OpenIdProvider) => {
    ...
    setZkAddress(zkLoginSetup.userAddr);
    const loginUrl = zkLoginSetup.demoLoginUrl();
      ...
      >
        <button
          onClick={async () => {
            ...
            const result = await moveCallSponsored(txb, account, false);
            if (result.effects?.status.status === "success") {
              setDigest(result.digest);
              const matchingObject = result.objectChanges?.find(
                // @ts-ignore
                (obj) => obj?.objectType === demoObjectType
              );
              // @ts-ignore
