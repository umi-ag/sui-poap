...
export function isPublisher(type: Type): boolean { return type === "0x2::package::Publisher"; }

export interface PublisherFields { id: string; package: string; moduleName: string }
...
export class Publisher { static readonly $typeName = "0x2::package::Publisher"; static readonly $numTypeParams = 0;
 ...
 } return new Publisher( { id: item.fields.id.id, package: item.fields.package, moduleName: item.fields.module_name } ) }

 static fromBcs( data: Uint8Array | string, encoding?: Encoding ): Publisher { return Publisher.fromFields( bcs.de([Publisher.$typeName, ], data, encoding) ) }

 static fromSuiParsedData(content: SuiParsedData) { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isPublisher(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a Publisher object`); } return Publisher.fromFieldsWithTypes(content); }

 static async fetch(client: SuiClient, id: string ): Promise<Publisher> { const res = await client.getObject({ id, options: { showContent: true, }, }); if (res.error) { throw new Error(`error fetching Publisher object at id ${id}: ${res.error.code}`); } if (res.data?.content?.dataType !== "moveObject" || !isPublisher(res.data.content.type)) { throw new Error(`object at id ${id} is not a Publisher object`); }
...
export function isUpgradeCap(type: Type): boolean { return type === "0x2::package::UpgradeCap"; }

export interface UpgradeCapFields { id: string; package: string; version: bigint; policy: number }
...
export class UpgradeCap { static readonly $typeName = "0x2::package::UpgradeCap"; static readonly $numTypeParams = 0;
 ...
 } return new UpgradeCap( { id: item.fields.id.id, package: item.fields.package, version: BigInt(item.fields.version), policy: item.fields.policy } ) }

 static fromBcs( data: Uint8Array | string, encoding?: Encoding ): UpgradeCap { return UpgradeCap.fromFields( bcs.de([UpgradeCap.$typeName, ], data, encoding) ) }

 static fromSuiParsedData(content: SuiParsedData) { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isUpgradeCap(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a UpgradeCap object`); } return UpgradeCap.fromFieldsWithTypes(content); }

 static async fetch(client: SuiClient, id: string ): Promise<UpgradeCap> { const res = await client.getObject({ id, options: { showContent: true, }, }); if (res.error) { throw new Error(`error fetching UpgradeCap object at id ${id}: ${res.error.code}`); } if (res.data?.content?.dataType !== "moveObject" || !isUpgradeCap(res.data.content.type)) { throw new Error(`object at id ${id} is not a UpgradeCap object`); }
...
export function isUpgradeTicket(type: Type): boolean { return type === "0x2::package::UpgradeTicket"; }

export interface UpgradeTicketFields { cap: string; package: string; policy: number; digest: Array<number> }
...
export class UpgradeTicket { static readonly $typeName = "0x2::package::UpgradeTicket"; static readonly $numTypeParams = 0;
 ...
 } return new UpgradeTicket( { cap: item.fields.cap, package: item.fields.package, policy: item.fields.policy, digest: item.fields.digest.map((item: any) => item) } ) }

 static fromBcs( data: Uint8Array | string, encoding?: Encoding ): UpgradeTicket { return UpgradeTicket.fromFields( bcs.de([UpgradeTicket.$typeName, ], data, encoding) ) }
...
export function isUpgradeReceipt(type: Type): boolean { return type === "0x2::package::UpgradeReceipt"; }

export interface UpgradeReceiptFields { cap: string; package: string }
...
export class UpgradeReceipt { static readonly $typeName = "0x2::package::UpgradeReceipt"; static readonly $numTypeParams = 0;
 ...
 static fromBcs( data: Uint8Array | string, encoding?: Encoding ): UpgradeReceipt { return UpgradeReceipt.fromFields( bcs.de([UpgradeReceipt.$typeName, ], data, encoding) ) }
