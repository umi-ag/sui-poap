...
export function isTxContext(type: Type): boolean { return type === "0x2::tx_context::TxContext"; }

export interface TxContextFields { sender: string; txHash: Array<number>; epoch: bigint; epochTimestampMs: bigint; idsCreated: bigint }
...
export class TxContext { static readonly $typeName = "0x2::tx_context::TxContext"; static readonly $numTypeParams = 0;
 ...
 } return new TxContext( { sender: `0x${item.fields.sender}`, txHash: item.fields.tx_hash.map((item: any) => item), epoch: BigInt(item.fields.epoch), epochTimestampMs: BigInt(item.fields.epoch_timestamp_ms), idsCreated: BigInt(item.fields.ids_created) } ) }

 static fromBcs( data: Uint8Array | string, encoding?: Encoding ): TxContext { return TxContext.fromFields( bcs.de([TxContext.$typeName, ], data, encoding) ) }
