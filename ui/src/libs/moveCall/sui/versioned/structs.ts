...
export function isVersionChangeCap(type: Type): boolean { return type === "0x2::versioned::VersionChangeCap"; }

export interface VersionChangeCapFields { versionedId: string; oldVersion: bigint }
...
export class VersionChangeCap { static readonly $typeName = "0x2::versioned::VersionChangeCap"; static readonly $numTypeParams = 0;
 ...
 } return new VersionChangeCap( { versionedId: item.fields.versioned_id, oldVersion: BigInt(item.fields.old_version) } ) }

 static fromBcs( data: Uint8Array | string, encoding?: Encoding ): VersionChangeCap { return VersionChangeCap.fromFields( bcs.de([VersionChangeCap.$typeName, ], data, encoding) ) }
...
export function isVersioned(type: Type): boolean { return type === "0x2::versioned::Versioned"; }

export interface VersionedFields { id: string; version: bigint }
...
export class Versioned { static readonly $typeName = "0x2::versioned::Versioned"; static readonly $numTypeParams = 0;
 ...
 static fromBcs( data: Uint8Array | string, encoding?: Encoding ): Versioned { return Versioned.fromFields( bcs.de([Versioned.$typeName, ], data, encoding) ) }

 static fromSuiParsedData(content: SuiParsedData) { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isVersioned(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a Versioned object`); } return Versioned.fromFieldsWithTypes(content); }

 static async fetch(client: SuiClient, id: string ): Promise<Versioned> { const res = await client.getObject({ id, options: { showContent: true, }, }); if (res.error) { throw new Error(`error fetching Versioned object at id ${id}: ${res.error.code}`); } if (res.data?.content?.dataType !== "moveObject" || !isVersioned(res.data.content.type)) { throw new Error(`object at id ${id} is not a Versioned object`); }
