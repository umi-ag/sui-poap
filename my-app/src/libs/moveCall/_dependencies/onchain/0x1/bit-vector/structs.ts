...
export function isBitVector(type: Type): boolean { return type === "0x1::bit_vector::BitVector"; }

export interface BitVectorFields { length: bigint; bitField: Array<boolean> }
...
export class BitVector { static readonly $typeName = "0x1::bit_vector::BitVector"; static readonly $numTypeParams = 0;
 ...
 } return new BitVector( { length: BigInt(item.fields.length), bitField: item.fields.bit_field.map((item: any) => item) } ) }

 static fromBcs( data: Uint8Array | string, encoding?: Encoding ): BitVector { return BitVector.fromFields( bcs.de([BitVector.$typeName, ], data, encoding) ) }
