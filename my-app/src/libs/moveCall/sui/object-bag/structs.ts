...
export function isObjectBag(type: Type): boolean { return type === "0x2::object_bag::ObjectBag"; }

export interface ObjectBagFields { id: string; size: bigint }
...
export class ObjectBag { static readonly $typeName = "0x2::object_bag::ObjectBag"; static readonly $numTypeParams = 0;
 ...
 static fromBcs( data: Uint8Array | string, encoding?: Encoding ): ObjectBag { return ObjectBag.fromFields( bcs.de([ObjectBag.$typeName, ], data, encoding) ) }

 static fromSuiParsedData(content: SuiParsedData) { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isObjectBag(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a ObjectBag object`); } return ObjectBag.fromFieldsWithTypes(content); }

 static async fetch(client: SuiClient, id: string ): Promise<ObjectBag> { const res = await client.getObject({ id, options: { showContent: true, }, }); if (res.error) { throw new Error(`error fetching ObjectBag object at id ${id}: ${res.error.code}`); } if (res.data?.content?.dataType !== "moveObject" || !isObjectBag(res.data.content.type)) { throw new Error(`object at id ${id} is not a ObjectBag object`); }
